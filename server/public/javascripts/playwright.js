...
async function getElementContentOrAttr() {
    const browser = await chromium.launch();
    const page = await browser.newPage();
    await page.goto('http://example.com');
...
(async () => {
    ...
    // 设置 cookies
    await setCookies(page, [{ name: 'test', value: '123', domain: 'example.com', path: '/' }]);
    // 获取所有 cookies
    const cookies = await getAllCookies(page);
    console.log(cookies);
    // 删除所有 cookies
    const numDeleted = await deleteAllCookies(page);
    console.log(`Deleted ${numDeleted} cookies.`);
...
(async () => {
    ...
    // 设置 localStorage
    await setLocalStorage(page, 'test', '123');
    // 获取所有 localStorage
...
async function handleMouseOrKeyboard() {
    const browser = await chromium.launch();
    const page = await browser.newPage();
    await page.goto('http://example.com');
...
async function monitorDownloadFile() {
    const browser = await chromium.launch();
    const page = await browser.newPage();
    await page.goto('http://example.com');
...
async function monitorPageJump() {
    const browser = await chromium.launch();
    const page = await browser.newPage();
...
async function interceptingNetworkRequests() {
    const browser = await chromium.launch();
    const page = await browser.newPage();
...
async function mockGeolocation() {
    ...
    // 设置地理位置
    await context.setGeolocation({ latitude: 52.52, longitude: 13.39 }); // Berlin
